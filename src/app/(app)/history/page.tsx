...
import { ChevronLeft, Star, Moon, BookOpen, Trash2, AlertTriangle, Plus, Calendar } from "lucide-react";
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    async function load() {
      const [sportsRes, settingsRes] = await Promise.all([fetch("/api/sports"), fetch("/api/settings")]);
  ...
  if (selectedDate) {
    ...
    return (
      <div className="animate-fade-in w-full max-w-full overflow-x-hidden">
        <header className="sticky top-0 z-30 flex items-center justify-between gap-2 border-b border-white/6 bg-[#0a0a0f]/80 px-4 py-4 pt-[max(env(safe-area-inset-top),1rem)] backdrop-blur-xl min-w-0">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={() => setSelectedDate(null)} className="flex-shrink-0 rounded-xl p-1.5 transition hover:bg-white/8">
              <ChevronLeft className="h-5 w-5 text-zinc-400" />
            </button>
            <div className="min-w-0">
              <h1 className="text-base font-bold text-white capitalize truncate">
                {format(parseISO(selectedDate), "EEEE d MMMM yyyy", { locale: it })}
              </h1>
              {isNew && <p className="text-xs text-indigo-400 mt-0.5">Nuova giornata</p>}
            </div>
          </div>
          {!isNew && (
            <button onClick={() => setConfirmDelete(selectedDate)}
              className="flex flex-shrink-0 items-center gap-1.5 rounded-xl border border-red-500/20 bg-red-500/10 px-3 py-1.5 text-xs font-semibold text-red-400 transition hover:bg-red-500/20">
              <Trash2 className="h-3.5 w-3.5" /> Elimina
            </button>
          )}
        </header>

        <DayForm date={selectedDate} initialDay={selectedDay} sports={sports} settings={settings}
          onSaved={async () => { await loadDays(); setToast({ message: "Giornata salvata! ✓", type: "success" }); }} />
        ...
        {confirmDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
            <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#13131a] p-6 animate-scale-in">
              ...
              <h3 className="text-center text-base font-bold text-white mb-1">Elimina giornata</h3>
              <p className="text-center text-sm text-zinc-400 mb-6">Sei sicuro? Questa azione non può essere annullata.</p>
  ...
  return (
    <div className="animate-fade-in w-full max-w-full overflow-x-hidden">
      ...
      <div className="space-y-6 px-4 pt-3 pb-4 min-w-0">
        <button onClick={() => setShowDatePicker(true)}
          ...
          <Plus className="h-4 w-4" /> Aggiungi giorno passato
        </button>
        ...
        {Array.from(grouped.entries()).map(([month, monthDays]) => (
          <div key={month} className="min-w-0">
            <h2 className="mb-2 px-1 text-xs font-bold uppercase tracking-widest text-zinc-600 capitalize">{month}</h2>
            <div className="space-y-2">
              {monthDays.map((d) => (
                <button key={d.id} onClick={() => setSelectedDate(d.date)}
                  className="block w-full min-w-0 text-left rounded-2xl border border-white/8 bg-white/4 p-4 transition hover:border-indigo-500/30 hover:bg-white/6 active:scale-[0.99]">
                  <div className="flex items-center justify-between gap-2 min-w-0">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="flex h-11 w-11 flex-shrink-0 flex-col items-center justify-center rounded-xl bg-indigo-500/10 border border-indigo-500/20">
                        <span className="text-[9px] font-bold uppercase text-indigo-400">{format(parseISO(d.date), "EEE", { locale: it })}</span>
                        <span className="text-lg font-bold leading-tight text-indigo-300">{format(parseISO(d.date), "d")}</span>
                      </div>
                      <div className="min-w-0">
                        <p className="truncate text-sm text-zinc-300 leading-snug">
                          {d.description.slice(0, 55)}{d.description.length > 55 ? "..." : ""}
                        </p>
                        <div className="mt-1 flex items-center gap-3 text-xs text-zinc-600 min-w-0">
                          <span className="flex items-center gap-1 flex-shrink-0">
                            <Moon className="h-3 w-3" />{d.sleepHours}h{d.sleepMinutes > 0 ? `${d.sleepMinutes}m` : ""}
                          </span>
                          <span className="flex items-center gap-1 flex-shrink-0"><BookOpen className="h-3 w-3" /> {d.pagesRead} pag</span>
                          {d.workouts.length > 0 && (
                            <span className="text-indigo-500 truncate">
                              {d.workouts.length === 1 ? d.workouts[0].sport.name : `${d.workouts.length} allenamenti`}
                            </span>
                          )}
                        </div>
                      </div>
      ...
      {showDatePicker && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-t-3xl border-t border-white/10 bg-[#13131a] p-6 animate-fade-in">
            ...
            <label className="block text-xs font-semibold uppercase tracking-widest text-zinc-500 mb-2">Seleziona la data</label>
            ...
            <div className="flex gap-2">
              <button onClick={() => { setShowDatePicker(false); setNewDate(""); }}
                className="flex-1 rounded-xl border border-white/10 bg-white/5 py-3 text-sm font-semibold text-zinc-300 transition hover:bg-white/10">
                Annulla
              </button>
              <button onClick={handleAddOldDay} disabled={!newDate}
